        ...
        {kohteet.map((kohde) => (
          ...
          >
            <span className="min-w-0 flex-1">
              <span className="block text-lg font-bold text-ink-strong truncate">{kohde.name}</span>
              {kohde.address && (
                <span className="block text-sm text-ink-muted mt-0.5 truncate">{kohde.address}</span>
              )}
            </span>
