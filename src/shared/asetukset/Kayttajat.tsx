...
import { UserPlus, KeyRound, Smartphone, LogOut, Copy, Check, AlertTriangle, Users } from 'lucide-react';
...
// Kertaalleen näytettävä salasana. Palvelin arpoo sen eikä tallenna selväkielisenä,
// joten tämä on ainoa hetki jolloin sen voi lukea — siksi se ei ole ohimenevä ilmoitus
// vaan laatikko joka suljetaan käsin.
type SalasanaNaytto = { username: string; password: string; syy: 'luotu' | 'nollattu' };
...
export const Kayttajat = ({ kayttajat, roles, isAdmin, puoli, onMuuttui }: Props) => {
  ...
  return (
    <div className="bg-surface rounded-xl border border-line shadow-sm p-6 mb-6 space-y-4">
      ...
      {/* Arvottu salasana. Näytetään kunnes pääkäyttäjä sulkee sen itse: tämä on ainoa
          kerta kun sen näkee, eikä katoava ilmoitus riitä. */}
      {salasana && (
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-warning-ink">
                {salasana.syy === 'luotu' ? 'Käyttäjä luotu' : 'Salasana nollattu'}: {salasana.username}
              </p>
              ...
              <div className="mt-2 flex items-center gap-2 flex-wrap">
                ...
                <button
                  type="button"
                  onClick={() => kopioi(salasana.password)}
